...
class RSU:
    ...
    line = f.readlines()[-1]  #Read the maximum and minimum values of each attribute returned from the DC
    mean_std = line.split('\t')  # Get μ and σ in max_min.csv file
    f.close()
    x_err_value = 2
    y_err_value = 4
    v_err_value = 4
    a_err_value = 0.8
    # The threshold that can be accepted for calculating z-score values for each attribute
    threshold = [3,3,3,3]
    ...
    def deal_record(self, record: dict):
        """
         Process the received record (with id as the key and the value as [t,x,y,v,a]), and store the record into the record by id.
         Check the last two reported times for records stored in record
         If the time is greater than 0.1 seconds, the historical data will be deleted and only the last reported information will be retained.
         :param record: #{id:[time,x,y,v,a,...]}
         :return: None, process self.his_record, no return value
         """
    ...
    def process_record(self, ve_id, now_time):
        """
         Process the record received by ve_id under now_time and return the correction plan
         :param ve_id: The vehicle ID that needs to be judged and corrected
         :param now_time: time when BSM was generated
         :return: Correction plan[0,0,0,0]
        """
    ...
    def judge_BSM(self, v_record: list):
        """
         Judge the received BSM by: z-score
         :param v_record: BSM generated by vehicle
         :return: Determination of vehicle BSM and incorrect attribute identification
         """
        """Judge the correctness of the changing values of attributes"""
        flag = 0
        err_arr = []
    ...
    def single_prediction(self, v_id, data_length=5):
        """
         Using historical vehicle data for single-vehicle prediction
         if len(history_data)-1 >= data_len then LSTM prediction correction
         else physics correction
         :param v_id: vehicle id
         :param data_len: the number of historical data used for prediction
         :return: correct the result
         """
    ...
    def single_correction(self, v_id, err_arr):
        """
         Bicycle prediction using vehicle kinematics laws
         :param v_id: vehicle id
         :param err_arr: List of attributes where errors occurred 1: Position; 2: Speed; 3: Acceleration
         :return: correct the result
         """
    ...
    def find_nei(self, v_id: str, now_time: str):
        """
         Find neighbors with v_id within self.nei_range
         :param v_id: vehicle id
    ...
    def multi_prediction(self, v_id: str, nei_id: list, err_arr: int, record_len=3):
        """
         Correction of v_id by neighbor
         :param v_id: vehicle id
         :param nei_id: list of neighbor IDs
         :param err_arr: error attribute identifier
         :param record_len: The historical BSM length used
         :return: Neighbor’s correction results for the vehicle
         """
    ...
    def corr_project(self, multi: list, now_data: list, v_id: str):
        """
         Fusion of correction results for single and multiple vehicles
         :param multi: multiple correction results
         :param now_data: reported BSM
