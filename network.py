# Data Center的功能
# 对数据进行归一化，计算各属性/样本间的相关性，确定相关度的上下限
#
import time
import numpy as np
import pandas as pd
...
class DataCenter:
    ...
    def train_model(self, record: dict):
        ...
        history = model.fit(test_X, test_y, validation_data=(val_x, val_y), epochs=250, batch_size=16)
        loss = history.history['loss']
        val_loss = history.history['val_loss']
        model.save(f"./result/model070411.h5")
        f = open('./result/loss070411.csv', 'a')
        f.write(str(loss))
        f.close()
        f2 = open('./result/val_loss070411.csv', 'a')
...
if __name__ == "__main__":
    dc = DataCenter()
    record = dc.dict2record('./NGSIM I80/data/data_train1.csv')
